...
n = int(sys.argv[1])
ten = np.zeros((n, n, n), dtype=np.uint8)
start = perf_counter()
write_numpy(ten, "numpy")
end = perf_counter()
print(f"Write time numpy: {end - start}")

start = perf_counter()
write_blosc(ten, "blosc")
end = perf_counter()
print(f"Write time blosc: {end - start}")

start = perf_counter()
read_numpy("numpy")
end = perf_counter()
print(f"Read time numpy: {end - start}")

start = perf_counter()
read_blosc("blosc")
end = perf_counter()
print(f"Read tim blosc: {end - start}")
